...
func (c *Client) processICMP(ctx context.Context, link *transport.Link, dialer internet.Dialer, fallbackDest xnet.Destination) error {
	if link == nil || link.Reader == nil || link.Writer == nil {
		return errors.New("trusttunnel icmp link is incomplete")
	}

	tunnelConn, err := c.connectICMPTunnel(ctx, dialer)
	if err != nil {
		return err
	}
...
func (c *Client) connectICMPTunnel(ctx context.Context, dialer internet.Dialer) (io.ReadWriteCloser, error) {
	...
	return c.connectWithEndpointPolicy(ctx, attempts, "ICMP endpoint", func(runCtx context.Context, attempt trustTunnelServerAttempt) (io.ReadWriteCloser, error) {
		...
		if attemptHTTP3 {
			...
			tunnelConn, err := trustTunnelConnectHTTP3Func(http3Ctx, serverAddr, trustTunnelICMPPseudoHost, account, c.config)
			cancelHTTP3()
			if err != nil {
				...
				errors.LogWarning(runCtx, "trusttunnel HTTP/3 ICMP CONNECT failed; falling back to HTTP/2 path: ", err)
			} else {
