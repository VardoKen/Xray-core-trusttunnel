...
- live preflight был таким:
  ...
  - runtime binary path: `/root/tt-multipath-phase3/xray-tt-multipath-linux-phase6`;
...
- Linux negative live reset-run дополнительно подтверждает:
  ...
  - authoritative negative rerun `/root/tt-multipath-phase3/logs/multipath-phase3-gap-20260413-204116` подтверждает реальный channel-loss path на одном alias IP и уже закрывает outer-layer surfacing: client runtime в `/root/tt-multipath-phase3/client-error.log` фиксирует `app/proxyman/outbound: ... trusttunnel connection ends > proxy/trusttunnel: trusttunnel multipath channel quorum lost`;
  - тот же rerun на server side фиксирует `trusttunnel multipath quorum grace expired > proxy/trusttunnel: trusttunnel multipath channel quorum lost: got 1, want 2 > proxy/trusttunnel: peer reported trusttunnel multipath channel loss` в `/root/tt-multipath-phase3/server-error.log`;
  - empty `client-gap-markers.txt` внутри bundle больше не трактовать как отсутствие marker'а в runtime: current phase3 gap harness всё ещё grep'ит `client.stdout.log`, тогда как live config пишет multipath markers в `client-error.log` / `server-error.log`;
...
- локально:
  ...
  - `$env:GOFLAGS='-buildvcs=false'; go test -run '^$' ./...`
  - `go build -buildvcs=false -o ./tmp/xray-tt-current.exe ./main`
  - `$env:GOOS='linux'; $env:GOARCH='amd64'; go build -buildvcs=false -o ./tmp/xray-tt-multipath-linux-phase6 ./main`
- live:
  - `XRAY_BIN=/root/tt-multipath-phase3/xray-tt-multipath-linux-phase4 bash /root/tt-multipath-phase3/live.sh`
  - `env XRAY_BIN=/root/tt-multipath-phase3/xray-tt-multipath-linux-phase6 /root/tt-multipath-phase3/multipath_phase3_gap_remote.sh`
