...
- локальная IPv4 validation теперь тоже закрыта: client bundles `/opt/lab/xray-tt/multipath-ipv4/logs/multipath-ipv4-positive-20260415-035212`, `/opt/lab/xray-tt/multipath-ipv4/logs/multipath-ipv4-rejoin-20260415-035247` и server bundles `/root/tt-multipath-ipv4/logs/multipath-ipv4-positive-20260415-035205`, `/root/tt-multipath-ipv4/logs/multipath-ipv4-rejoin-20260415-035235` подтверждают `8-of-8` positive и восстановление `7/8 -> 8/8` на destination IPv4 `192.168.1.50` ... `192.168.1.57`.
- ordinary single-path compatibility тоже уже закрыта: client bundle `/opt/lab/xray-tt/compat-ipv4/logs/multipath-ipv4-single-20260415-034757` и server bundle `/root/tt-multipath-ipv4/logs/multipath-ipv4-positive-20260415-034747` подтверждают, что без `multipath.enabled` TrustTunnel остаётся обычным H2/TLS CONNECT path.
...
- fresh shared-prod `8-of-8` positive smoke на code state `b280ad5d` через client bundle `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-positive-20260415-035505` и server bundle `/root/tt-multipath-prod/logs/multipath-ipv6-positive-20260415-035455` подтверждает, что close-lifecycle hardening не переоткрыл внешний regress;
