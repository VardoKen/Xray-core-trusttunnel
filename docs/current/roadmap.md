...
Открытым остаётся:
- завершение `_icmp` config surface: уже wired `settings.allowPrivateNetworkConnections`, `settings.icmp.interfaceName`, `settings.icmp.requestTimeoutSecs`, H2 lab runtime уже подтвердил private-network policy и invalid `icmp.interfaceName`, но отсутствует аналог official `recv_message_queue_capacity`;
- отдельный lab/runtime-retest для `icmp.requestTimeoutSecs`;
...
1. `_icmp` config surface и error-type parity
