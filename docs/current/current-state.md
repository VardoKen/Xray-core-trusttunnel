...
На ветке `feat/trusttunnel-multipath` открыта новая experimental-R&D линия TrustTunnel Multipath Transport. На текущем этапе она всё ещё не изменила это подтверждённое runtime-состояние stable TrustTunnel path, но уже вышла в phase 6 outer-layer quorum-loss surfacing и закрыла локальную multi-host IPv4/IPv6 валидацию:
...
- `proxy/trusttunnel/multipath_client.go` и `proxy/trusttunnel/multipath_frame.go` уже дают H2/TLS multipath payload runtime с dynamic channel set: writer переживает write-fail канала и продолжает на surviving channels, request-side tunnel copy делает explicit `CloseWrite()` после EOF, reorder path вместо мгновенного overflow-fail использует bounded backpressure до закрытия gap или timeout, peer channel-loss может быть surfaced control-frame `channel_closed`, strict quorum-loss уходит на peer через session-closing control-frame, а ACK / resend control-path вместе с sender-side in-flight window удерживают WAN rejoin от повторного `reorder gap timed out` / `reorder window exceeded`;
...
- authoritative negative Linux live validation через bundle `/root/tt-multipath-phase3/logs/multipath-phase3-gap-20260413-204116` подтверждает channel-loss path и закрывает outer-layer quorum-loss surfacing: `drop.log` фиксирует `drop-mode=nft`, downstream long download рвётся с `curl: (18) end of response ... missing`, client runtime в `/root/tt-multipath-phase3/client-error.log` фиксирует `app/proxyman/outbound: ... trusttunnel connection ends > proxy/trusttunnel: trusttunnel multipath channel quorum lost`, а server runtime в `/root/tt-multipath-phase3/server-error.log` фиксирует `trusttunnel multipath quorum grace expired ... trusttunnel multipath channel quorum lost`;
...
- host-to-host IPv6 positive validation между Linux lab `192.168.1.19` и второй VM `192.168.1.25` подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv6/logs/multipath-ipv6-positive-20260414-002036` и server bundle `/root/tt-multipath-ipv6/logs/multipath-ipv6-positive-20260414-002024`: один client source address `fd42:5940:deef::19` одновременно держит два канала к server destination IP `fd42:5940:deef::50` и `fd42:5940:deef::51`, а staged-download SHA-256 совпадает с ожидаемым `79cf58c41ad3d94d7b41c668dfb378899d2cc70b6a28736122c1331626476731`;
- host-to-host IPv6 negative validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv6/logs/multipath-ipv6-gap-20260414-002600` и server bundle `/root/tt-multipath-ipv6/logs/multipath-ipv6-gap-20260414-002549`: `nft reject with tcp reset` на канале через `fd42:5940:deef::51` приводит к `curl: (18) end of response with 8388608 bytes missing`, client-side marker `trusttunnel connection ends > proxy/trusttunnel: trusttunnel multipath channel quorum lost`, а server-side marker фиксирует `trusttunnel multipath quorum grace expired ... trusttunnel multipath channel quorum lost`;
- host-to-host IPv6 recovery/rejoin validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv6/logs/multipath-ipv6-rejoin-20260414-002736` и server bundle `/root/tt-multipath-ipv6/logs/multipath-ipv6-rejoin-20260414-002725`: после временного drop канала на `fd42:5940:deef::51` quorum деградирует до `1/2`, затем восстанавливается, client log фиксирует `trusttunnel multipath rejoined endpoint [fd42:5940:deef::51]:9543 ... channel=3`, а download SHA-256 снова совпадает с ожидаемым;
- host-to-host IPv6 `4-of-4` positive validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv6/logs/multipath-ipv6-positive-20260414-004247` и server bundle `/root/tt-multipath-ipv6/logs/multipath-ipv6-positive-20260414-004234`: тот же один client source address `fd42:5940:deef::19` одновременно держит уже четыре канала к `fd42:5940:deef::50`, `::51`, `::52`, `::53`, server log фиксирует attach `channel=2/3/4`, а staged-download SHA-256 остаётся целым;
- host-to-host IPv6 `4-of-4` recovery/rejoin validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv6/logs/multipath-ipv6-rejoin-20260414-004428` и server bundle `/root/tt-multipath-ipv6/logs/multipath-ipv6-rejoin-20260414-004416`: после временного drop канала на `fd42:5940:deef::53` quorum деградирует до `3/4`, затем восстанавливается, client log фиксирует `trusttunnel multipath rejoined endpoint [fd42:5940:deef::53]:9543 ... channel=5`, а download SHA-256 снова совпадает с ожидаемым;
- host-to-host IPv6 `8-of-8` recovery/rejoin validation после WAN-hardening подтверждена через client bundle `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-localvm8rejoin-20260415-012238` и server bundle `/root/tt-multipath-ipv6/logs/multipath-ipv6-rejoin-20260415-012237`: один client source address `fd42:5940:deef::19` держит восемь каналов к `fd42:5940:deef::50` ... `::57`, после временного drop канала на `::57` quorum деградирует до `7/8`, затем восстанавливается, client log фиксирует `trusttunnel multipath rejoined endpoint [fd42:5940:deef::57]:9543 ... channel=9`, а staged-download SHA-256 `7dc0246dec155ee4e23864818b275e4438cba45e0740383d05014242d541bdd7` совпадает с ожидаемым;
- external/public IPv6 positive validation больше не ограничена локальной парой Linux VM: на shared host `flyingamaranth.aeza.network` через isolated high-port `:9543` и без касания `443` / `rw-core` подтверждены `8-of-8` и `16-of-16` positive runs между lab и prod host;
- external `8-of-8` positive validation подтверждена через client bundles `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-prod8-20260414-011910` и `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-prod8b-20260414-012103` вместе с server bundle `/root/tt-multipath-prod/logs/multipath-ipv6-positive-20260414-011854`: один lab client source address `2a00:1370:81a2:2918:bd5:c785:b185:6de4` держит восемь одновременных TCP channels к `2a12:5940:deef::50` ... `::57`, `curl.exitcode = 0`, SHA-256 staged-download совпадает, lab `ss-9543.txt` фиксирует `8` established channels, а prod `server-error.log` фиксирует attach до `channel=8`;
- external `16-of-16` positive validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-prod-ipv6-16/logs/multipath-ipv6-prod16-20260414-012526` и server bundle `/root/tt-multipath-prod/logs/multipath-ipv6-positive-20260414-012325`: тот же один lab client source address держит уже `16` одновременных TCP channels к `2a12:5940:deef::50` ... `::5f`, `curl.exitcode = 0`, SHA-256 совпадает, lab `ss-9543.txt` фиксирует `16` established channels, а prod `server-error.log` фиксирует attach до `channel=16`;
- external/shared-prod IPv6 `8-of-8` recovery/rejoin validation теперь тоже закрыта: client bundle `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-prod8rejoinfix-20260415-012434` и server bundle `/root/tt-multipath-prod/logs/multipath-ipv6-rejoin-20260415-012434` подтверждают, что после временного drop канала на `2a12:5940:deef::57` quorum деградирует до `7/8`, затем восстанавливается, client log фиксирует `trusttunnel multipath rejoined endpoint [2a12:5940:deef::57]:9543 ... channel=9`, `curl.exitcode = 0`, а staged-download SHA-256 `7dc0246dec155ee4e23864818b275e4438cba45e0740383d05014242d541bdd7` совпадает с ожидаемым;
- локальная IPv4 `8-of-8` positive validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv4/logs/multipath-ipv4-positive-20260415-035212` и server bundle `/root/tt-multipath-ipv4/logs/multipath-ipv4-positive-20260415-035205`: один client source address `192.168.1.19` одновременно держит восемь TCP channels к `192.168.1.50` ... `192.168.1.57`, `curl.exitcode = 0`, staged-download SHA-256 `79cf58c41ad3d94d7b41c668dfb378899d2cc70b6a28736122c1331626476731` совпадает с ожидаемым, а server log фиксирует attach до `channel=8` без ложного post-completion quorum-loss tail;
- локальная IPv4 `8-of-8` recovery/rejoin validation подтверждена через client bundle `/opt/lab/xray-tt/multipath-ipv4/logs/multipath-ipv4-rejoin-20260415-035247` и server bundle `/root/tt-multipath-ipv4/logs/multipath-ipv4-rejoin-20260415-035235`: `drop.log` фиксирует `drop-mode=nft` и `drop-removed=1`, client log фиксирует `trusttunnel multipath quorum degraded ... got=7 want=8`, затем `trusttunnel multipath quorum restored ... channels=8` и `trusttunnel multipath rejoined endpoint 192.168.1.57:9543 ... channel=9`, а staged-download SHA-256 остаётся целым;
- ordinary single-path compatibility после multipath-ветки подтверждена live через client bundle `/opt/lab/xray-tt/compat-ipv4/logs/multipath-ipv4-single-20260415-034757` и server bundle `/root/tt-multipath-ipv4/logs/multipath-ipv4-positive-20260415-034747`: без `multipath.*` outbound по-прежнему работает как обычный H2/TLS TrustTunnel CONNECT path, `curl.exitcode = 0`, staged-download SHA-256 совпадает, а server log содержит `trusttunnel H2 CONNECT accepted for tcp:127.0.0.1:18080`;
- свежий shared-prod IPv6 `8-of-8` positive smoke на code state `b280ad5d` подтверждён через client bundle `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-positive-20260415-035505` и server bundle `/root/tt-multipath-prod/logs/multipath-ipv6-positive-20260415-035455`: `curl.exitcode = 0`, staged-download SHA-256 `79cf58c41ad3d94d7b41c668dfb378899d2cc70b6a28736122c1331626476731` совпадает, server log фиксирует attach до `channel=8`, а ложные close-time `quorum degraded` markers на client side больше не воспроизводятся.
- H1 и H3 pseudo-host path для multipath по-прежнему честно режутся как unsupported;
- следующей открытой фазой остаётся уже не локальная IPv6 валидация, не первый external positive verdict и не baseline external `8-of-8` rejoin, а более жёсткая higher-cardinality external validation: `16+` / `32+` / `50+` recovery-rejoin, load/CPU и, при необходимости, более агрессивный fault-injection на dedicated multi-IP host вне shared prod VM.
