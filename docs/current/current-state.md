...
На ветке `feat/trusttunnel-multipath` открыта новая experimental-R&D линия TrustTunnel Multipath Transport. На текущем этапе она всё ещё не изменила это подтверждённое runtime-состояние stable TrustTunnel path, но уже вышла в phase 6 outer-layer quorum-loss surfacing:
...
- следующей открытой фазой остаётся отдельная external multi-IP validation вне локальной Linux VM.
