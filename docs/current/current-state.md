...
- следующей открытой фазой остаётся уже не локальная или первая external positive validation, а отдельная external fault-injection / recovery / rejoin validation на dedicated multi-IP host вне shared prod VM; на shared prod host текущий этап сознательно ограничен positive runs без firewall/route вмешательства.
