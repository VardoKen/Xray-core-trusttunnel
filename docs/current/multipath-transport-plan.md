...
- один логический клиентский session context использует несколько серверных destination IP одновременно, оставаясь при этом на одном client source address;
...
Исходная идея уже задаёт правильную TCP-модель: одна логическая сессия должна использовать несколько отдельных TCP-соединений на разные серверные IP одновременно, при этом клиент остаётся на одном своём source address.

Для этой ветки важно зафиксировать точную трактовку фразы «сервер обязан отвечать со случайного IP из пула»:
- на уровне логической multipath-сессии сервер вправе использовать разные IP из пула для разных TCP-каналов;
- для каждого отдельного уже установленного TCP-соединения source IP остаётся фиксированным;
- multipath-свойство достигается распределением framed session-data по нескольким TCP-каналам, а не переносом одного уже установленного TCP-канала между IP.
...
- multipath для TCP должен строиться на множестве одновременных TCP-соединений;
- каждое отдельное TCP-соединение отвечает со своего local IP;
- сервер объединяет эти TCP-каналы в одну session-level модель и обрабатывает их как одну логическую сессию.
...
- UDP;
- `_icmp`;
- попытку перекидывать уже установленный TCP-канал на другой IP;
...
5. держать TCP-модель в рамках нескольких отдельных каналов на разные IP внутри одной сессии;
