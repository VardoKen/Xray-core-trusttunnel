...
- experimental `multipath.*` config surface уже существует вместе с `_mptcp_open` / `_mptcp_attach` control path, H2/TLS framed payload runtime, phase-5 recovery/rejoin path и phase-6 outer-layer quorum-loss surfacing; current code уже держит dynamic channel snapshot, per-channel accounting, bounded reorder backpressure, explicit strict quorum-loss semantics, request-side half-close, peer `channel_closed` / `session_closing` control-frame, ACK / resend control-path, sender-side in-flight window, close-lifecycle suppression для ложного post-completion quorum-loss tail и detached session runtime через `context.WithoutCancel(...)`, а live bundles `/root/tt-multipath-phase3/logs/multipath-phase3-live-20260413-092248`, `/root/tt-multipath-phase3/logs/multipath-phase5-rejoin-20260413-194749`, `/root/tt-multipath-phase3/logs/multipath-phase3-gap-20260413-204116`, `/opt/lab/xray-tt/multipath-ipv4/logs/multipath-ipv4-positive-20260415-035212`, `/opt/lab/xray-tt/multipath-ipv4/logs/multipath-ipv4-rejoin-20260415-035247`, `/opt/lab/xray-tt/multipath-ipv6/logs/multipath-ipv6-positive-20260414-002036`, `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-localvm8rejoin-20260415-012238`, `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-prod8rejoinfix-20260415-012434` и `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-positive-20260415-035505` подтверждают working multi-IP payload path, strict quorum-loss и rejoin как на IPv4 destination IP `192.168.1.50` ... `192.168.1.57`, так и на IPv6 destination IP `fd42:5940:deef::50/51/52/53/54/55/56/57` и `2a12:5940:deef::50` ... `::57` при одном client source address; multipath остаётся opt-in surface и не меняет ordinary single-path CONNECT path без `multipath.enabled=true`, а открытыми остаются уже только higher-cardinality external recovery/load задачи
...
- `multipath.*` на текущем этапе уже является phase-6 experimental runtime surface: config/validator, session registry, `_mptcp_open`, `_mptcp_attach`, attach-proof, client/server framed payload layer, server-side payload dispatch после channel quorum, dynamic writer retry по surviving channels, explicit strict quorum-loss error, peer-visible session-closing marker и recovery/rejoin уже существуют;
...
- shared-prod external `8-of-8` rejoin уже закрыт через bundle `/opt/lab/xray-tt/multipath-prod-ipv6/logs/multipath-ipv6-prod8rejoinfix-20260415-012434`; оставшимися открытыми задачами остаются higher-cardinality external rejoin и load/CPU.
