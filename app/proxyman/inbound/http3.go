...
func isHTTP3Capable(p proxy.Inbound) bool {
	_, ok := p.(http3Capable)
	return ok
}
...
type http3CountReadCloser struct {
	io.ReadCloser
	counter stats.Counter
}

func (r *http3CountReadCloser) Read(p []byte) (int, error) {
...
type http3CountResponseWriter struct {
	http.ResponseWriter
	counter stats.Counter
}

func (w *http3CountResponseWriter) Write(p []byte) (int, error) {
...
func (w *http3CountResponseWriter) Flush() {
	if f, ok := w.ResponseWriter.(http.Flusher); ok {
		f.Flush()
	}
}

type http3Worker struct {
	address         net.Address
	port            net.Port
	proxy           proxy.Inbound
...
func (w *http3Worker) Proxy() proxy.Inbound {
	return w.proxy
}

func (w *http3Worker) Port() net.Port {
	return w.port
}

func (w *http3Worker) Start() error {
	h3proxy, ok := w.proxy.(http3Capable)
	if !ok {
		return errors.New("proxy does not support http3 inbound")
	}

	tlsCfgObj := itls.ConfigFromStreamSettings(w.stream)
	if tlsCfgObj == nil {
		return errors.New("tls config is nil for http3 inbound")
	}

	tlsCfg := tlsCfgObj.GetTLSConfig()
	if tlsCfg == nil {
		return errors.New("tls config is empty for http3 inbound")
	}
	...
	handler := http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		if w.downlinkCounter != nil {
			rwCounted = &http3CountResponseWriter{
				ResponseWriter: rw,
	...
	go func() {
		for {
			...
			if err != nil {
				errors.LogInfoInner(w.ctx, err, "http3 listener closed")
				return
			}

			go func() {
				if err := w.server.ServeQUICConn(conn); err != nil {
					errors.LogDebugInner(w.ctx, err, "http3 connection ended")
				}
...
func (w *http3Worker) Close() error {
	var errs []interface{}
	...
	if len(errs) > 0 {
		return errors.New("failed to close http3 worker").Base(errors.New(errs...))
	}
